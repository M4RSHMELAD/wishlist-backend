...
import (
	...
	notificationrepo "github.com/qwersedzxc/wishlist-backend/internal/repository/notification"
...
func Run(ctx context.Context, cfg *config.Config, db *database.Database, log *slog.Logger) error {
	...
	// Инициализация notification компонентов
	notificationRepo := notificationrepo.New(db.Pool)
	notificationUC := notificationuc.New(notificationRepo, log)
	...
	wishlistUC := wishlistuc.New(wishlistRepo, wishlistItemRepo, notificationRepo, userRepo, log)
	...
	router := v1.NewRouter(wishlistUC, authUC, friendshipUC, notificationUC, roleRepo, oauthProvider, cfg.OAuth.Provider, cfg.S3, emailService, log, cfg)
