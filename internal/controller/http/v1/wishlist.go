...
func (h *WishlistHandler) ReserveItem(w http.ResponseWriter, r *http.Request) {
	...
	var req struct {
		...
		HideReserverName bool `json:"hideReserverName"`
	}
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		// Если body пустой, используем false по умолчанию
		req.IsIncognito = false
		req.HideReserverName = false
	}

	if err := h.uc.ReserveItem(r.Context(), itemID, userID, req.IsIncognito, req.HideReserverName); err != nil {
