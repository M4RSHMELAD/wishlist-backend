...
func (h *AuthHandler) GetSettings(w http.ResponseWriter, r *http.Request) {
	...
	render.JSON(w, r, map[string]interface{}{
		...
		"showFulfilledWishes": u.ShowFulfilledWishes,
	})
}
...
func (h *AuthHandler) UpdateSettings(w http.ResponseWriter, r *http.Request) {
	...
	var input struct {
		...
		ShowFulfilledWishes *bool `json:"showFulfilledWishes"`
	}
	if err := json.NewDecoder(r.Body).Decode(&input); err != nil {
		render.Status(r, http.StatusBadRequest)
		render.JSON(w, r, response.Error("invalid request body"))
	...
	err = h.authUC.UpdateSettings(r.Context(), userID, input.PushNotifications, input.EmailNotifications, input.ShowFulfilledWishes)
	if err != nil {
		render.Status(r, http.StatusInternalServerError)
		render.JSON(w, r, response.Error(err.Error()))
		return
