...
func New(repo Repository, itemRepo ItemRepository, notificationRepo NotificationRepository, userRepo UserRepository, log *slog.Logger) *UseCase {
	return &UseCase{
		repo:             repo,
		itemRepo:         itemRepo,
...
func (uc *UseCase) ReserveItem(ctx context.Context, itemID, userID uuid.UUID, isIncognito bool, hideReserverName bool) error {
	...
	// Создаем уведомление для владельца вишлиста
	var message string
	var relatedUserIDPtr *uuid.UUID
	
	if isIncognito {
		// Инкогнито: полностью скрываем информацию
		message = "Кто-то забронировал один из ваших подарков"
	} else if hideReserverName {
		// Скрыть имя: показываем что подарок забронирован, но не показываем кто
		message = "Кто-то забронировал один из ваших подарков"
	} else {
		// Показывать имя: получаем информацию о пользователе
		relatedUserIDPtr = &userID
		
		reservingUser, err := uc.userRepo.GetByID(ctx, userID)
		if err != nil {
			uc.log.ErrorContext(ctx, "failed to get reserving user", "userID", userID, "error", err)
			message = fmt.Sprintf("Пользователь забронировал ваш подарок - %s", item.Title)
		} else {
			// Используем FullName если есть, иначе Username
			userName := ""
			if reservingUser.FullName != nil && *reservingUser.FullName != "" {
				userName = *reservingUser.FullName
			} else {
				userName = reservingUser.Username
			}
			message = fmt.Sprintf("%s забронировал(а) ваш подарок - %s", userName, item.Title)
		}
	}

	notification := entity.Notification{
		UserID:        wishlist.UserID,
		Type:          entity.NotificationTypeGiftReserved,
		Title:         "Подарок забронирован",
		Message:       message,
		RelatedItemID: &itemID,
		RelatedUserID: relatedUserIDPtr,
		IsRead:        false,
	}

	if err := uc.notificationRepo.Create(ctx, notification); err != nil {
		// Логируем ошибку, но не прерываем операцию бронирования
		uc.log.ErrorContext(ctx, "failed to create notification", "error", err)
	}

	uc.log.InfoContext(ctx, "item reserved", "itemID", itemID, "userID", userID, "isIncognito", isIncognito, "hideReserverName", hideReserverName)
	return nil
}
